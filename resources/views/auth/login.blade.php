    ...
    <div class="flex min-h-screen flex-col lg:flex-row">
        ...
        <main class="w-full lg:w-[55%] flex flex-col justify-between bg-white p-8 sm:p-12 md:p-16 lg:p-24 min-h-screen">
            ...
            <div class="my-auto py-10 max-w-sm w-full mx-auto space-y-6">
                <div>
                    <h1 class="text-3xl font-extrabold tracking-tight text-slate-950">Welcome back</h1>
                    <p class="mt-2 text-sm text-slate-500">
                        No account? <a href="{{ route('register') }}" class="font-semibold text-emerald-600 hover:underline">Register here</a>
                    </p>
                </div>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative mt-1">
                            <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus placeholder="alex@company.com" class="block w-full rounded-lg border-slate-200 py-3 pl-10 text-sm shadow-sm focus:border-[#7FA8BA] focus:ring-[#7FA8BA] border">
                    ...
                    <div class="flex items-center justify-between text-xs pt-1">
                        <label for="remember_me" class="flex items-center gap-2 cursor-pointer">
                            <input id="remember_me" type="checkbox" name="remember" class="rounded border-slate-300 text-[#2F5F73] focus:ring-[#7FA8BA]">
                            <span class="text-slate-500 font-medium">Remember for 30 days</span>
                        </label>
                        @if (Route::has('password.request'))
                            <a href="{{ route('password.request') }}" class="font-semibold text-[#2F5F73] hover:underline">Forgot password?</a>
                        @endif
                    </div>
                    ...
                    <p class="text-center text-xs text-slate-400 pt-1">Click <span class="font-semibold text-slate-500">Sign In</span> to enter.</p>
                </form>
            </div>
