<div class="space-y-5">
    ...
    <div>
        ...
        <input id="title" name="title" type="text" value="{{ old('title', $milestone?->title) }}" required class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-[#7FA8BA] focus:ring-[#7FA8BA]">
        <x-input-error :messages="$errors->get('title')" class="mt-2" />
    </div>

    <div>
        <label for="description" class="block text-sm font-semibold text-slate-700">Description</label>
        <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-[#7FA8BA] focus:ring-[#7FA8BA]">{{ old('description', $milestone?->description) }}</textarea>
    </div>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
        ...
        <div>
            ...
            <input id="due_date" name="due_date" type="date" value="{{ old('due_date', $milestone?->due_date) }}" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm focus:border-[#7FA8BA] focus:ring-[#7FA8BA]">
        </div>
    </div>

    <div class="flex items-center gap-3 pt-2">
        <button type="button" class="rounded-lg bg-[#2F5F73] px-4 py-2 text-sm font-semibold text-white hover:bg-[#244B5C]">Save Milestone</button>
